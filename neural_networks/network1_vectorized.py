...
class Network(object):
    ...
    def backprop(self, mini_batch):
        ...
        :param mini_batch: 
        :return: tuple (nabla_b, nabla_w) containing gradients of cost function
        """
        ...
        x_matr = np.concatenate([mb[0] for mb in mini_batch], axis=1)
        y_matr = np.concatenate([mb[1] for mb in mini_batch], axis=1)
        ...
        nabla_b[-1] = delta.sum(axis=1).reshape( (len(nabla_b[-1]), 1) )
        nabla_w[-1] = np.sum(np.einsum('ik,jk->kji', activations[-2], delta), axis=0)

        for l in xrange(2, self.num_layers):
            delta = np.dot(self.weights[-l + 1].transpose(), delta) * sigmoid_prime(zs[-l])
            nabla_b[-l] = delta.sum(axis=1).reshape( (len(nabla_b[-l]), 1) )
            nabla_w[-l] = np.sum(np.einsum('ik,jk->kji', activations[-l-1], delta), axis=0)
